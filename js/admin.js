...
async function cargarUsuario(user) {
  ...
  if (email === SUPERADMIN_EMAIL || user.uid === SUPERADMIN_UID) {
    usuarioActualData = {
      uid: user.uid, nombre: "Facundo Emmert", email, rol: "superadmin", activo: true,
  ...
  if (invitacion.exists() && invitacion.data().activo === true) {
    ...
    usuarioActualData = { uid: user.uid, nombre: data.nombre || email, email, rol: "admin", activo: true, permisos: data.permisos || {} };
    await setDoc(doc(db, "admins", user.uid), { ...usuarioActualData, actualizado: serverTimestamp() }, { merge: true });
    return true;
  }
  return false;
...
onAuthStateChanged(auth, async (user) => {
  ...
  document.getElementById("usuarioActual").textContent = `${usuarioActualData.nombre || user.email} - ${usuarioActualData.rol}`;
...
function ocultarSeccionesSinPermiso() {
  document.querySelectorAll("[data-permiso]").forEach(btn => {
    if (!tienePermiso(btn.dataset.permiso)) btn.remove();
  });
  document.querySelectorAll("[data-panel-permiso]").forEach(panel => {
    if (!tienePermiso(panel.dataset.panelPermiso)) panel.remove();
  });
...
function iniciarConfiguracion() {
  ...
  onSnapshot(configRef, documento => {
    ...
    if (data.logoUrl) { logoAdmin.src = data.logoUrl; logoAdmin.classList.remove("oculto"); }
  });
...
function iniciarObjetivos() {
  ...
  form.addEventListener("submit", async e => {
    ...
    form.reset(); document.getElementById("recaudado").value = 0;
  });
...
function renderObjetivosAdmin() {
  ...
  objetivosCache.filter(o => `${o.nombre || ""} ${o.descripcion || ""}`.toLowerCase().includes(busqueda)).forEach(o => {
    const id = o.id, precio = Number(o.precio || 0), recaudado = Math.min(Number(o.recaudado || 0), precio);
    const faltante = Math.max(0, precio - recaudado), completado = precio > 0 && recaudado >= precio;
...
window.mostrarEditor = id => { document.getElementById("vista-" + id).classList.add("oculto"); document.getElementById("editor-" + id).classList.remove("oculto"); };
window.cancelarEditor = id => { document.getElementById("editor-" + id).classList.add("oculto"); document.getElementById("vista-" + id).classList.remove("oculto"); };
window.guardarEdicion = async id => {
  ...
  await updateDoc(doc(db, "objetivos", id), {
    ...
    precio, recaudado: Math.min(recaudado, precio),
...
window.eliminarObjetivo = async id => { if (!confirm("¿Seguro que querés eliminar este objetivo?")) return; await deleteDoc(doc(db, "objetivos", id)); };

function iniciarDonaciones() {
  if (!tienePermiso("objetivos")) return;
  const lista = document.getElementById("listaDonacionesPendientes");
  if (!lista) return;

  onSnapshot(query(collection(db, "donaciones")), snapshot => {
    lista.innerHTML = "";
    snapshot.forEach(d => {
      const don = d.data();
      if (don.estado !== "pendiente") return;
      const div = document.createElement("div");
      div.className = "admin-card";
      div.innerHTML = `
        <div>
          <h3>${don.objetivoNombre || "Objetivo"}</h3>
          <p><strong>Donante:</strong> ${don.nombreDonante}</p>
          <p><strong>Monto:</strong> ${formatoPesos.format(Number(don.monto || 0))}</p>
          <p><strong>Estado:</strong> Pendiente</p>
          <p><a href="${don.comprobanteUrl}" target="_blank" rel="noopener">Ver comprobante</a></p>
        </div>
        <div class="admin-actions">
          <button onclick="aprobarDonacion('${d.id}', '${don.objetivoId}', ${Number(don.monto || 0)})">Aprobar</button>
    ...
    if (!lista.innerHTML) {
      lista.innerHTML = "<p>No hay donaciones pendientes.</p>";
    }
  });
}

window.aprobarDonacion = async (donacionId, objetivoId, monto) => {
  if (!confirm("¿Confirmás que esta transferencia fue verificada?")) return;
  await updateDoc(doc(db, "objetivos", objetivoId), { recaudado: increment(monto), actualizado: serverTimestamp() });
  await updateDoc(doc(db, "donaciones", donacionId), {
    estado: "aprobada",
    aprobadoPor: usuarioActualData.email,
    aprobado: serverTimestamp()
  });
  alert("Donación aprobada y sumada al objetivo.");
};

window.rechazarDonacion = async (donacionId) => {
  if (!confirm("¿Rechazar esta donación pendiente?")) return;
  await updateDoc(doc(db, "donaciones", donacionId), {
    estado: "rechazada",
    rechazadoPor: usuarioActualData.email,
    rechazado: serverTimestamp()
  });
};

function iniciarObra() {
  if (!tienePermiso("obra")) return;
  const form = document.getElementById("obraForm"), lista = document.getElementById("listaObra");
  ...
  onSnapshot(query(collection(db, "obra")), snapshot => {
    lista.innerHTML = "";
    snapshot.forEach(d => {
      const o = d.data(), div = document.createElement("div");
      div.className = "card";
...
window.eliminarObra = async id => { if (!confirm("¿Eliminar esta actualización?")) return; await deleteDoc(doc(db, "obra", id)); };

function iniciarTransparencia() {
  if (!tienePermiso("transparencia")) return;
  const form = document.getElementById("transparenciaForm"), lista = document.getElementById("listaTransparenciaAdmin");
  if (!form || !lista) return;
  form.addEventListener("submit", async e => {
    e.preventDefault();
...
window.eliminarTransparencia = async id => { if (!confirm("¿Eliminar este comprobante?")) return; await deleteDoc(doc(db, "transparencia", id)); };

function permisosComoTexto(permisos = {}) {
  const nombres = { institucional: "Información institucional", objetivos: "Objetivos y donaciones", obra: "Actualización de obra", transparencia: "Transparencia", usuarios: "Administradores" };
  const activos = Object.entries(permisos).filter(([k, v]) => v).map(([k]) => nombres[k] || k);
  return activos.length ? activos.join(", ") : "Sin permisos asignados";
}

function iniciarUsuarios() {
  if (!tienePermiso("usuarios")) return;
  const form = document.getElementById("usuarioForm"), lista = document.getElementById("listaUsuarios");
  if (!form || !lista) return;
  ...
  form.addEventListener("submit", async e => {
    ...
    await setDoc(doc(db, "adminsByEmail", email), {
      ...
      autorizadoPor: usuarioActualData.email, creado: serverTimestamp()
    });
    form.reset();
  ...
  onSnapshot(query(collection(db, "admins")), snapshot => {
    adminsCreados = {};
    snapshot.forEach(d => { const u = d.data(); if (u.email) adminsCreados[u.email.toLowerCase()] = { id: d.id, ...u }; });
    renderAdmins();
  });
  onSnapshot(query(collection(db, "adminsByEmail")), snapshot => {
    adminsAutorizados = {};
    snapshot.forEach(d => { const u = d.data(); if (u.email) adminsAutorizados[u.email.toLowerCase()] = { id: d.id, ...u }; });
    renderAdmins();
  });
}
...
function renderAdmins() {
  ...
  emails.forEach(email => {
    const autorizado = adminsAutorizados[email], creado = adminsCreados[email], data = creado || autorizado;
    const esSuper = data.rol === "superadmin" || email === SUPERADMIN_EMAIL;
    const permisos = data.permisos || {}, estado = data.activo !== false;
    const div = document.createElement("div");
    div.className = "admin-card";
    div.innerHTML = `
      <div>
...
window.cambiarEstadoAdmin = async (email, estado) => { await setDoc(doc(db, "adminsByEmail", email), { activo: estado }, { merge: true }); };
window.editarPermisosAdmin = (email) => { document.getElementById("edit-admin-" + email.replaceAll("@", "_").replaceAll(".", "_")).classList.toggle("oculto"); };
window.guardarPermisosAdmin = async (email) => {
  ...
  if (creado && creado.id) await setDoc(doc(db, "admins", creado.id), { permisos, actualizado: serverTimestamp() }, { merge: true });
